...
rcl_publisher_t publisher_callback;
...
void timer_callback(rcl_timer_t * timer, int64_t last_call_time) {
  ...
  if (timer != NULL) {
    RCSOFTCHECK(rcl_publish(&publisher_callback, &pub_msg, NULL));
...
void setup() {
  ...
  pub_msg.data.capacity = 3; // or however many elements you need
  pub_msg.data.size = 3;
  pub_msg.data.data = (float*)malloc(3 * sizeof(float));
  
  // Populate data
  pub_msg.data.data[0] = 0.0f; // example values
